...
```
type ProductsInBasket = Pick<IProductItem, 'id' | 'title' | 'price' | 'isAddedToBasket'>
```
...
#### Class AppApi
Наследует методы от базового класса api. Используется для конкретизации в методах запрашиваемых данных с сервера. Класс имеет 3 метода: 
getProductsList() - получает список товаров с сервера. 
getProductItem() - получает конкретный товар с сервера.
sendDataOrder() - отправляет запрос на сервер. В теле запроса содержит данные о выбранных товарах в корзине и о введенных в формы данных пользователя.
...
changedProductList() - когда изменяются данные в корзине, вызывется этот метод, который создает событие изменения в корзине.
...
Конструктор принимает container: HTMLElement и объект с событием. Передает container и событие в родительский конструктор, сохраняет нужные элементы разметки в поля класса. Вешает слушатель на кнопку оплаты. с которым работает сеттер payment. Устанавливает класс на активную кнопку;

Методы:
ressetButtons() - сбрасывает состояние кнопки в неактивное положение;
set adress - устанавливает значение поля адрес;
selectPayment(method: PayOptions) - класс для выбора кнопки способа оплаты;
onInputChange() - сигнализирует брокер о произошедшем событии и передаем вместе с событием данные.
...
## Слой Presenter
В слое Presenter находится связующая часть между объектами Model и View. Слой презентер представляет брокер событий EventEmitter. Объект брокера инициализируется в index.ts и методы класса EventEmitter используются для прослушивания событий, которые происходят в приложении и таким образом происходит связь между моделью и представлением. 
